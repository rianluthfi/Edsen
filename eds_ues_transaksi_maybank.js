...
function(record, serverWidget, runtime) {
    ...
    function afterSubmit(context) {
		...
		if (context.type === context.UserEventType.CREATE || context.type === context.UserEventType.EDIT){
			var Maybank = record.load({
				type : 'customtransaction_eds_transaction_mayban',
				id : context.newRecord.id,
				isDynamic : true
			});
				
			var tranId = Maybank.getValue({
				fieldId : 'tranid'
			});
			
			var number = 1000 + tranId;
			
			var tranDate = Maybank.getValue({
				fieldId : 'trandate'
			});
			...
			Maybank.setValue({
				fieldId : 'tranid',
				value :'B3/EGK/'+monthToRomawi(MM)+'/'+YY+'/'+number.slice(-3)
			});
