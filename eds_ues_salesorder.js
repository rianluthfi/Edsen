...
function(record, serverWidget, runtime) {
    ...
    function beforeLoad(context) {
		...
		var isAmor = form.getField('custbody_eds_auto_journal_ammor');
		
		log.debug('tigeer');
		isAmor.updateDisplayType({displayType : serverWidget.FieldDisplayType.NORMAL});
    ...
    function afterSubmit(context) {
		...
		if (context.type === context.UserEventType.CREATE || context.type === context.UserEventType.EDIT){
			...
			SalesOrder.setValue({
				fieldId : 'tranid',
				value : 'SO/EGK/'+monthToRomawi(MM)+'/'+YY+'/'+tranId.slice(-3)
			});
		...
		if (context.type === context.UserEventType.CREATE || context.type === context.UserEventType.EDIT){
			...
			var tax10 = subtotal*0.1
			var total = parseInt(subtotal) + parseInt(tax10);
			var idSO = context.newRecord.id;
			
			var masterSO = record.load({
				type: record.Type.SALES_ORDER, 
				id: idSO,
				isDynamic: true,
			});
			//masterSO.setValue('custbody_eds_in_word', terbilang(total));
			masterSO.setValue('custbody_eds_in_word', toTitleCase(int_to_words(total))+' Rupiah');
			masterSO.save();
		}
	}
