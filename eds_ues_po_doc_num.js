    ...
    function afterSubmit(context) {
		...
		if (context.type === context.UserEventType.CREATE || context.type === context.UserEventType.EDIT) {
			...
			purchaseOrder.setValue({
				fieldId : 'tranid',
				value : 'PO/EGK/'+monthToRomawi(MM)+'/'+YY+'/'+tranId.slice(-3)
			});
		...
		if (context.type === context.UserEventType.CREATE || context.type === context.UserEventType.EDIT){
			...
			//Terbilang Normal
			masterPO.setValue('custbody_eds_in_word', inWord(total)+' '+inCurrency(currency));
			
			//Terbilang PPN
			// var totalPPN = subtotal + (subtotal * 0.01);
			masterPO.setValue('custbody_eds_in_word_ppn', inWord(subtotal)+' '+inCurrency(currency));
			masterPO.save();
		}
	}
