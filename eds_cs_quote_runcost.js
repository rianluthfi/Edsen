...
define(['N/currentRecord', 'N/record','N/format'],
    function(currentRecord, record, format) {
        function pageInit(context) {
			...
			var opportunityId = curRec.getValue('opportunity');
			var discPerc = curRec.getValue('custbody_eds_discount_perc');
			var discNom = curRec.getValue('custbody_eds_discount_nominal');
			
			if (opportunityId != '' && discPerc == '' && discNom == ''){
				var masOpp = record.load({
					type : record.Type.OPPORTUNITY,
					id : opportunityId,
					isDynamic : true
				});
				
				var projectedTotal = masOpp.getValue('projectedtotal');
				curRec.setValue('custbody_eds_projected_total', projectedTotal);
				curRec.setValue('custbody_eds_contract_value', projectedTotal);
			}
        }
        function saveRecord(context) {
			
			return true;//vicky overwrite other functions
			var curRec = context.currentRecord;			
