...
function(record, serverWidget, runtime) {
    ...
    function afterSubmit(context) {
		...
		if (context.type === context.UserEventType.CREATE || context.type === context.UserEventType.EDIT){
			var check = record.load({
				type : record.Type.CHECK,
				id : context.newRecord.id,
				isDynamic : true
			});
				
			var tranId = check.getValue({
				fieldId : 'tranid'
			});
			
			var number = 1000 + tranId;
			
			var tranDate = check.getValue({
				fieldId : 'trandate'
			});
			
			var account = check.getValue({
				fieldId : 'account'
			});
			
			log.debug('nilai account : '+account);
			...
			log.debug(tranId + ' '+setPrefix(account));
			
			check.setValue({
				fieldId : 'tranid',
				value : setPrefix(account)+'/EGK/'+monthToRomawi(MM)+'/'+YY+'/'+number.slice(-3)
			});
