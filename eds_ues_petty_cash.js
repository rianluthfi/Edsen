...
function(record, serverWidget, runtime) {
    ...
    function afterSubmit(context) {
		...
		if (context.type === context.UserEventType.CREATE || context.type === context.UserEventType.EDIT){
			var pettyCash = record.load({
				type : 'customtransaction_eds_petty_cash',
				id : context.newRecord.id,
				isDynamic : true
			});
				
			var tranId = pettyCash.getValue({
				fieldId : 'tranid'
			});
			
			var number = 1000 + tranId;
			
			var tranDate = pettyCash.getValue({
				fieldId : 'trandate'
			});
			...
			pettyCash.setValue({
				fieldId : 'tranid',
				value :'KK/EGK/'+monthToRomawi(MM)+'/'+YY+'/'+number.slice(-3)
			});
