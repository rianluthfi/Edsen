...
function(record, serverWidget, runtime) {
    ...
    function afterSubmit(context) {
		...
		if (context.type === context.UserEventType.CREATE || context.type === context.UserEventType.EDIT){
			var masterPay = record.load({
				type : record.Type.CUSTOMER_PAYMENT,
				id : context.newRecord.id,
				isDynamic : true
			});
				
			var tranId = masterPay.getValue({
				fieldId : 'tranid'
			});
			
			var tranDate = masterPay.getValue({
				fieldId : 'trandate'
			});
			...
			masterPay.setValue({
				fieldId : 'tranid',
				value : 'CUSPAY/EGK/'+monthToRomawi(MM)+'/'+YY+'/'+tranId.slice(-3)
			});
		...
		if (context.type === context.UserEventType.CREATE || context.type === context.UserEventType.EDIT){
			var total = newRecord.getValue('payment');
			...
			masterInv.setValue('custbody_eds_in_word', int_to_words(total)+' Rupiah');
			masterInv.save();
		}
	}
